...
{
	#define MAX_AUTH_LOGON_CHALLENGES_IN_A_ROW 2
    ...
    while (1)
    {
        ...
        if (i == AUTH_TOTAL_COMMANDS)
        {
         ...
         if (_cmd == AUTH_LOGON_CHALLENGE)
         {
             ...
             {
                 sLog->outDebug(LOG_FILTER_NETWORKIO, "Got %u AUTH_LOGON_CHALLENGE in a row from '%s', possible ongoing DDoS", challengesInARow, socket().getRemoteAddress().c_str());
                 socket().shutdown();
                 return;
             }
         }
