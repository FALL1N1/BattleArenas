            ...
            SpellCastResult CheckCast()
            {
                ...
                if (GetTargetUnit())
                    if (Player* target = GetTargetUnit()->ToPlayer())
						// Fix Holy Shock after Mind Control in arena, 
...
{	
    public:	
        ...
        {	
            ...
            SpellCastResult CheckCast()	
            {					
				// Prevent Divine Shield when Cyclone is active
                if (GetCaster()->HasAura(SPELL_CYCLONE))
                    return SPELL_FAILED_STUNNED;

				if (GetCaster()->HasAura(SPELL_FORBEARANCE))
...
//INSERT INTO `spell_script_names` (`spell_id`, `ScriptName`) VALUES (-1022, 'spell_pal_hand_of_protection');
...
{	
    public:	
        ...
        {	
            ...
            SpellCastResult CheckCast()	
            {	
				// Prevent self-cast for Hand of Protection when Forbearance is up
				// Only on Party members if forberance is applied to paladin:
                if (GetTargetUnit()->HasAura(SPELL_FORBEARANCE))
                    return SPELL_FAILED_TARGET_AURASTATE;

				if (GetCaster()->HasAura(SPELL_BLIND))
...
//INSERT INTO `spell_script_names` (`spell_id`, `ScriptName`) VALUES (498, 'spell_pal_divine_protection');
...
{	
    public:	
        ...
        {	
            ...
            SpellCastResult CheckCast()	
            {	
				// Prevent Divine Protection when Forbearance is up
                if (GetCaster()->HasAura(SPELL_FORBEARANCE))
                    return SPELL_FAILED_TARGET_AURASTATE;					
                return SPELL_CAST_OK;	
            }	
            ...
            void Register()	
            {	
                OnCheckCast += SpellCheckCastFn(spell_pal_divine_protection_SpellScript::CheckCast);	
            }	
        };	
...
// Fix LoH exploit with forbearance
// INSERT INTO `spell_script_names` (`spell_id`, `ScriptName`) VALUES (-633, 'spell_pal_lay_on_hands');
...
{	
    public:	
        ...
        {	
            ...
            SpellCastResult CheckCast()	
            {	
				// Prevent Lay on Hands when Forbearance is up
				// Only on targets without Forbearance aura:
                if (GetTargetUnit()->HasAura(SPELL_FORBEARANCE))
                    return SPELL_FAILED_TARGET_AURASTATE;					
                return SPELL_CAST_OK;	
            }	
