...
class spell_item_flag_of_ownership : public SpellScriptLoader
{
    public:
        spell_item_flag_of_ownership() : SpellScriptLoader("spell_item_flag_of_ownership") {}

        class spell_item_flag_of_ownership_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_item_flag_of_ownership_SpellScript);
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
				...
				Trinity::AnyDeadUnitSpellTargetInRangeCheck check(caster, max_range, GetSpellInfo(), TARGET_CHECK_ENEMY);
                Trinity::WorldObjectSearcher<Trinity::AnyDeadUnitSpellTargetInRangeCheck> searcher(caster, corpse, check);
                caster->GetMap()->VisitFirstFound(caster->m_positionX, caster->m_positionY, max_range, searcher);

				float x = corpse->GetPositionX();
				float y = corpse->GetPositionY();
				float z = corpse->GetPositionZ();
				float ang = corpse->GetOrientation();
				float rot2 = std::sin(ang/2); 
				float rot3 = std::cos(ang/2);

				caster->SummonGameObject(190589, x, y, z, ang, 0, 0, rot2, rot3, 30);;
            }

            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_item_flag_of_ownership_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
                OnCheckCast += SpellCheckCastFn(spell_item_flag_of_ownership_SpellScript::CheckIfCorpseNear);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_item_flag_of_ownership_SpellScript();
        }
};
...
void AddSC_item_spell_scripts()
{
	...
	new spell_item_flag_of_ownership();
}
