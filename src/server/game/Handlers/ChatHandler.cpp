    ...
    switch (type)
    {
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            if (!receiver || (senderIsPlayer && !receiverIsPlayer && !receiver->isAcceptWhispers() && !receiver->IsInWhisperWhiteList(sender->GetGUID())))
            {
                ...
                { 
					// TODO:
					//ChatHandler(GetPlayer()).PSendSysMessage("%s wishes to not be disturbed and cannot receive whisper messages: Busy", to.c_str());
					ChatHandler(GetPlayer()).PSendSysMessage("Player wishes to not be disturbed and cannot receive whisper messages: Busy");
                    return; 
                } 
                else 
                { 
