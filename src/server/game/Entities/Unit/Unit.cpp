...
void Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
	...
	// All tradeskill abilities instant cast
	spell->prepare(&targets, triggeredByAura, (
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit, int32 calc_resist)
{
	...
	if (damage > 0)
	{
		...
		{
			// Chaos Bolt - Cannot be resisted, and 
			// pierces through all absorption effects.
		case 3178: // Chaos Bolt
			break;
		default:
...
void Unit::AttackerStateUpdate (Unit* victim, WeaponAttackType attType, bool extra)
{
	...
	if (!victim->isAlive())
		return;
	// pets should respect line of sight
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
	...
	if (!victim->HasInArc(M_PI,this) && !victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION) && !victim->HasAura(19263)) // Deterrence can parry from behind
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
	...
	if (attType == RANGED_ATTACK)
	{
		...
		if (spellInfo->Category == SPELLCATEGORY_JUDGEMENT)
			return SPELL_MISS_NONE;
	...
	if (!victim->HasInArc(M_PI, this))
	{
		...
		else // Only deterrence as of 3.3.5
		{
			...
			{
				// Additionally by Natureknight (prevent dodge shred, backstab, ambush etc.)
				canParry = false;
				canDodge = false;
				canBlock = false;
			}
...
uint32 Unit::CalcMagicSpellHitChance(Unit* victim, SpellSchoolMask schoolMask, SpellInfo const* spellInfo)
{
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
	...
	if (!victim->HasAuraType(SPELL_AURA_MOD_STUN) && !(spellInfo->AttributesEx3 & SPELL_ATTR3_IGNORE_HIT_RESULT) && deflect_chance > 0)
		...
		// 100% chance for Cloak of Shadows to resist spell
		if (victim->HasAura(31224)) // CoS
			rand = 0;

		if (rand < miss)
		{
			// Cloak of Shadows should return resist instead of miss
			if (victim->HasAura(31224)) // CoS
				return SPELL_MISS_RESIST;
...
{
	// Workaround for Dispel Magic VS Divine Shield/Ice Block
	if ((victim->HasAura(642) || victim->HasAura(45438)) // Divine Shield and Ice Block
		// Dispel Magic all ranks, Devour Magic
			&& (spellInfo->Id == 527 || spellInfo->Id == 988 || spellInfo->Id == 48011))
			...
			case 31801:
				{
					...
					/*bool stacker = !procSpell || procSpell->Id == 53595;*///Old one
	...
	case SPELLFAMILY_SHAMAN:
		{
			...
			case 70817:
				...
				// Fix for totem of wrath: https://github.com/Xanvial/TrinityCore/commit/0bc77ea087654117b1c7799949e53119bab6f78b #1
	...
	case SPELLFAMILY_DEATHKNIGHT:
		{
			...
			if (dummySpell->Id == 49028)
			{
				...
				for (ControlList::const_iterator itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr) // Find Rune Weapon
					...
					else if (pPet && pPet->getVictim() && damage && !procSpell)     // copy 50% melee damage
					{
						CalcDamageInfo damageInfo;
				...
				case 67758: // Item - Coliseum 25 Heroic Caster Trinket
					{
						...
						// Fix item procs of Reign of the Dead / Reign of the Unliving, cooldown should be used
						// Link: https://github.com/Elron103/TrinityCore/commit/bed5746477f025beb810499d028cfcea71bf99d8
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
	...
	else if (victim->ToCreature()->IsInEvadeMode())
		return false;
...
void Unit::SetMinion(Minion *minion, bool apply)
{
	...
	if (apply)
	{
		...
		if (minion->IsPetGhoul())
		{
			...
			// Set starting energy for Risen Ghoul to 100 by Natureknight:
			minion->SetPower(POWER_ENERGY,  minion->GetMaxPower(POWER_ENERGY));
			// Risen Ghoul should not be fearable by Natureknight:
			minion->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_FEAR, true);
			minion->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_HORROR, true);
		}
...
void Unit::SetCharm(Unit* charm, bool apply)
{
	if (apply)
	{
		...
		// walk bug after mind control fix
...
uint32 Unit::SpellDamageBonus(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
	...
	for (AuraEffectList::const_iterator j = ResIgnoreAuras.begin(); j != ResIgnoreAuras.end(); ++j)
		if (TakenTotalMod < 1.0f)
			TakenTotalMod += (1.0f - TakenTotalMod) / 2.0f;
...
uint32 Unit::SpellHealingBonus(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
	...
	// Remove the twice heal reduction
	switch (spellProto->Id)
	{
	case 33778: // Lifebloom final bloom
		return healamount;
		break;
	case 63544: // Empowered Renew proc on spell cast
		// should heal 15% of the amount of the renew ticks
		return healamount /= 2;
		break;
	}
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
	...
	// fix some spell immunes
	if (GetTypeId() == TYPEID_PLAYER && spellInfo->Id != 49560 && spellInfo->Id != 62124)
	{
...
void Unit::MeleeDamageBonus(Unit* victim, uint32 *pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
	...
	for (AuraEffectList::const_iterator j = ResIgnoreAuras.begin(); j != ResIgnoreAuras.end(); ++j)
		if (TakenTotalMod < 1.0f)
			TakenTotalMod += (1.0f - TakenTotalMod) / 2;
...
void Unit::SetInCombatWith(Unit* enemy)
{
	...
	//if (eOwner->IsPvP())
	if (eOwner->GetTypeId() == TYPEID_PLAYER) // from Styler (fix combat bug)
	{
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
	...
	// can't assist invisible - https://github.com/TrinityCore/TrinityCore/issues/6077 must return TRUE isntead of FALSE:
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
	...
	for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
	{
		...
		bool active = damage || (procExtra & (PROC_EX_ABSORB | PROC_EX_BLOCK));
		if (isVictim)
	...
	for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
	{
		...
		// Prevent proc these spells from full absorbs
		switch (i->aura->GetId())
		{
		case 41635: // Prayer of Mending (all ranks)
		case 48110:
		case 48111:
		case 31876: // Judgements of the Wise (all ranks)
		case 31877:
		case 31878:
		case 48168: // Inner Fire (all ranks)
		case 48040:
		case 25431:
		case 10952:
		case 10951:
