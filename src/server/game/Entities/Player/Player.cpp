	...
	if (m_MirrorTimerFlags & (UNDERWATER_INLAVA /*| UNDERWATER_INSLIME*/) && !(_lastLiquid && _lastLiquid->SpellId))
	{
		...
		else
		{
			...
			if (m_MirrorTimer[FIRE_TIMER] < 0)
			{
				...
				// Fix for damaging attacks in Ruins of Lordaeron
...
{
	// Fix arena starting place when feared bug
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
	...
	// Fix exploit on Change faction while dead (Night Elf)
	...
	// Revive player at max health and mana
	...
	// Cast resurection sickness to target resurrected by spirit healer
	// this will prevent honor farming
	CastSpell(this, 46705, true); // Honorless Target
	// Remove all spell cooldowns on revive
...
void Player::RepopAtGraveyard()
{
	...
	// Custom graveyard coordinates for PVP zone (Jaguero Isle):
	if (GetAreaId() == 297)
	{
		int spawnLocations = urand(0, 10); // randomize spawn locations
		...
		case 0: // 1 
			cMap = 0;             // Map ID
			cx = -14608.195313f;  // x coords
			cy = -420.679626f;    // y coords
			cz = 4.330451f;       // z coords
			co = 0.738f; 
			break;
		case 1: // 2 
			cMap = 0;             // Map ID
			cx = -14781.799805f;  // x coords
			cy = -350.007385f;    // y coords
			cz = 28.329025f;       // z coords
			co = 3.556798f;
			break;
		case 2: // 3
			cMap = 0;             // Map ID
			cx = -14756.873047f;  // x coords
			cy = -291.798248f;    // y coords
			cz = 38.250771f;       // z coords
			co = 0.102692f;
			break;
		case 3: // 4
			cMap = 0;             // Map ID
			cx = -14735.481445f;  // x coords
			cy = -198.964f;    // y coords
			cz = 6.070635f;       // z coords
			co = 3.7134f;
			break;
		case 4: // 5
			cMap = 0;             // Map ID
			cx = -14700.421875f;  // x coords
			cy = -108.377487f;    // y coords
			cz = 1.957032f;       // z coords
			co = 4.421853f;
			break;
		case 5: // 6
			cMap = 0;             // Map ID
			cx = -14606.958984f;  // x coords
			cy = -231.344315f;    // y coords
			cz = 24.047932f;       // z coords
			co = 5.431098f;
			break;
		case 6: // 7
			cMap = 0;             // Map ID
			cx = -14527.342773f;  // x coords
			cy = -159.966751f;    // y coords
			cz = 6.320098f;       // z coords
			co = 4.088f;
			break;
		case 7: // 8
			cMap = 0;             // Map ID
			cx = -14506.407227f;  // x coords
			cy = -310.961792f;    // y coords
			cz = 2.190016f;       // z coords
			co = 3.231994f;
			break;
		case 8: // 9
			cMap = 0;             // Map ID
			cx = -14642.300781f;  // x coords
			cy = -299.498871f;    // y coords
			cz = 27.553432f;       // z coords
			co = 3.392149f;
			break;
		case 9: // 10
			cMap = 0;             // Map ID
			cx = -14650.909180f;  // x coords
			cy = -263.820618f;    // y coords
			cz = 49.753433f;       // z coords
			co = 0.541148f;
			break;
		case 10: // 11
			cMap = 0;             // Map ID
			cx = -14705.614258f;  // x coords
			cy = -302.059113f;    // y coords
			cz = 55.903671f;       // z coords
			co = 1.134121f;
			break;
			}
			break;
	...
	// Additional check for dueling zone (Ashenvale)
	if (GetAreaId() == 331)  
	{
		uint32 cMap;
		float cx;
		...
		{
			cMap = 1;               // Map ID
			cx = 2295.693604f;      // x coords
			cy = -651.048645f;      // y coords
			cz = 119.683044f;       // z coords
			co = 1.582438f;         // o coords
		}
	...
	// Additional check for VIP island
	if (GetAreaId() == 2317)  // South seas
	{
		uint32 cMap;
		float cx;
		float cy;
		...
		{
			cMap = 1;               // Map ID
			cx = -11781.913086f;    // x coords
			cy = -4783.916992f;     // y coords
			cz = 12.638429f;        // z coords
			co = 2.291762f;         // o coords
		}
	...
	// Additional check for Silvermyst Isle - PvP Zone
	if (GetZoneId() == 3524) // Azuremyst Isle
	{
		uint32 cMap;
		float cx;
		float cy;
		...
		{
			cMap = 530;               // Map ID
			cx = -4898.210449f;    // x coords
			cy = -10847.208008f;     // y coords
			cz = 0.921585f;        // z coords
			co = 3.762266f;         // o coords
		}

		if (GetTeam() == HORDE)
		{
			cMap = 530;               // Map ID
			cx = -5216.513184f;    // x coords
			cy = -11219.827148f;     // y coords
			cz = 40.364941f;        // z coords
			co = 0.625307f;         // o coords
		}
	...
	// Additional check for Goldenmist Village - PvP Zone
	if (GetZoneId() == 3433) // Ghostlands
	{
		uint32 cMap;
		float cx;
		float cy;
		...
		{
			cMap = 530;               // Map ID
			cx = 7739.864258f;    // x coords
			cy = -6075.500488f;     // y coords
			cz = 19.238945f;        // z coords
			co = 6.100757f;         // o coords
		}

		if (GetTeam() == HORDE)
		{
			cMap = 530;               // Map ID
			cx = 8001.195801f;    // x coords
			cy = -6227.183105f;     // y coords
			cz = 22.003067f;        // z coords
			co = 2.688986f;         // o coords
		}
	...
	// Additional check for Alcaz Isle - World PVP Zone
	if (GetAreaId() == 2079)  
	{
		uint32 cMap;
		float cx;
		...
		{
			cMap = 1;               // Map ID
			cx = -2703.838135f;     // x coords
			cy = -4692.713867f;     // y coords
			cz = 10.251527f;        // z coords
			co = 4.718532f;         // o coords
		}
		else if (GetTeam() == HORDE)
		{
			cMap = 1;               // Map ID
			cx = -2792.665771f;      // x coords
			cy = -4935.417969f;      // y coords
			cz = 71.483627f;       // z coords
			co = 4.106722f;         // o coords
		}
...
{
	// Allow players to join LFG channel everywhere 
...
void Player::RewardReputation(Unit* victim, float rate)
{
	...
	{
		int32 donerep1 = CalculateReputationGain(victim->getLevel(), Rep->RepValue1, ChampioningFaction ? ChampioningFaction : repfaction1, false);
	...
	{
		int32 donerep2 = CalculateReputationGain(victim->getLevel(), Rep->RepValue2, ChampioningFaction ? ChampioningFaction : repfaction2, false);
...
void Player::CastItemUseSpell(Item* item, SpellCastTargets const& targets, uint8 cast_count, uint32 glyphIndex)
{
	...
	if (proto->Spells[0].SpellId == 483 || proto->Spells[0].SpellId == 55884)
	{
		...
		spell->prepare(&targets, NULL, instant); // instant glyphs
		return;
	}
...
{
	// always pvp on in battleground or arena
...
void Player::learnDefaultSpells()
{
	...
	{
		uint32 tspell = *itr;
		sLog->outDebug(LOG_FILTER_PLAYER_LOADING, "PLAYER (Class: %u Race: %u): Adding initial spell, id = %u", uint32(getClass()), uint32(getORace()), tspell);
				...
				for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)                  // search through the SpellInfo for valid trigger spells
					// Mage's Combustion
						if (_spellEntry->Effects[i].TriggerSpell > 0 && (_spellEntry->Effects[i].Effect == SPELL_EFFECT_LEARN_SPELL || _spellEntry->Effects[i].Effect == SPELL_EFFECT_TRIGGER_SPELL))
						{
							...
							RemoveAurasDueToSpell(_spellEntry->Effects[i].TriggerSpell);
						}
		}
	}
