...
#define MAX_QUIET_DISTANCE 33.0f 
...
void FleeingMovementGenerator<T>::_setTargetLocation(T &owner)
{
    ...
    path.setPathLengthLimit(MAX_QUIET_DISTANCE);	
    ...
    i_nextCheckTime.Reset(traveltime + urand(200, 250)); 
}
...
bool FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
    ...
    i_nextCheckTime.Reset(urand(100, 500));	
    return false;	
}	
...
void FleeingMovementGenerator<T>::Initialize(T &owner)
{
	...
	owner.StopMoving(); // Decreasing fear bug by Natureknight
