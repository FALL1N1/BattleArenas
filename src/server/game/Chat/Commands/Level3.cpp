...
bool ChatHandler::HandleReviveCommand(const char *args)
{
    ...
    if (target)
    {
        target->ResurrectPlayer(!AccountMgr::IsPlayerAccount(target->GetSession()->GetSecurity()) ? 1.0f : /*0.5*/1.0f);
...
bool ChatHandler::HandleBanCharacterCommand(const char *args)
{
   ...
   char* reason = strtok(NULL, "");
    ...
    switch (sWorld->BanCharacter(name, duration, reason, m_session ? m_session->GetPlayerName() : ""))
    {
        ...
        {
            if (atoi(duration) > 0)
                sWorld->SendWorldText(LANG_CHAR_BANMESSAGE_WORLD, name.c_str(), secsToTimeString(TimeStringToSecs(duration), true).c_str(), reason);
            else
                sWorld->SendWorldText(LANG_CHAR_PERMABANMESSAGE_WORLD, name.c_str(), reason);
            break;
        }
        case BAN_NOTFOUND:
        {
...
bool ChatHandler::HandleBanHelper(BanMode mode, const char *args)
{
    ...
    switch (sWorld->BanAccount(mode, nameOrIP, duration, reason, m_session ? m_session->GetPlayerName() : ""))
    {
        case BAN_SUCCESS:
            if (atoi(duration)>0)
				sWorld->SendWorldText(LANG_ACC_BANMESSAGE_WORLD, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(duration), true).c_str(), reason);
            else
				sWorld->SendWorldText(LANG_ACC_PERMABANMESSAGE_WORLD, nameOrIP.c_str(), reason);
            break;
