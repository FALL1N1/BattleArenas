...
void BattlegroundDS::PostUpdateImpl(uint32 diff)
{
	...
	{
	    case BG_DS_WATERFALL_STATUS_ON:
		{
			if (getWaterFallKnockbackTimer() < diff) // Repeat knockback while the waterfall still active
		    {
	    ...
	    case BG_DS_WATERFALL_STATUS_WARNING:
		{
			if (GameObject* gob = GetBgMap()->GetGameObject(BgObjects[BG_DS_OBJECT_WATER_1]))
	    ...
	    case BG_DS_WATERFALL_STATUS_OFF:
		{
