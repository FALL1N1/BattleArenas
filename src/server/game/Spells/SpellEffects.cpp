		...
		case SPELLFAMILY_GENERIC:
			{
				...
				case 61547: // krystallus shatter ( Heroic )
					...
					// Hand of Reckoning (trigger)
				case 67485:
					{
						// Damage only if target is not player and when target is not targeting you
...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
	...
	if (OldSummon)
	{
		//Reset Pet CD After summon by Natureknight
	...
	case SPELLFAMILY_WARRIOR:
		{
			...
			if (m_spellInfo->SpellFamilyFlags[0] & 0x8000000) // Mocking Blow should not deal damage to players
			{	
			...
			case 50725:
				{
					...
					/*unitTarget->ToPlayer()->RemoveSpellCooldown(355, true);*/ // Old one
					if (m_originalCaster)	
