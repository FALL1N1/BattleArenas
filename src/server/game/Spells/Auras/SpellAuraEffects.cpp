...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        // apply glow vision
		// Remove the "blurry" effect from Arena Preparation Spell
        // if (target->GetTypeId() == TYPEID_PLAYER) - old one
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
	...
	// Deathbringer's Will procs for feral forms:
	if (target->HasAuraType(SPELL_AURA_TRANSFORM) && m_spellInfo->IsPositive() && !target->IsPolymorphed() 
		&& target->getClass() == CLASS_DRUID) // Prevent wow error when rogue with DBW use FoK in stealth
	{
		modelid = target->getTransForm();
	}
	else
	{
    ...
    if (apply)
    {
		...
		// Vanish should not break Shadow Dance
...
void AuraEffect::HandleAuraTransform(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
		...
		// Added  || GetSpellInfo()->IsPositive() for deathbringer's will procs with feral forms
        if (!target->GetModelForForm(target->GetShapeshiftForm()) || !GetSpellInfo()->IsPositive() || GetSpellInfo()->IsPositive())
        {
        ...
        if (apply)
        {
            ...
            switch (GetId())
            {
                ...
                case 52916: // Honor Among Thieves
                   ...
                   break;
				   //Sacred Shield swing timer reset by Natureknight:
...
void AuraEffect::HandlePeriodicDummyAuraTick(Unit* target, Unit* caster) const
{
    switch (GetSpellInfo()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            {
                case 49016: // Hysteria
					// prevent wpe exploit for hysteria
                    if (target->GetTypeId() != TYPEID_PLAYER)
                    return;
