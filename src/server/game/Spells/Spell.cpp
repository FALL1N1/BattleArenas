...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
	...
	// fixed totems to force them not spawn in walls or pillars
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    // Spell Effects Dispel (should put caster in combat also by Kandera)
                        ...
                        {	
                            ...
                            {	
                              switch (m_diminishLevel)	
                               {	
								   // This fixes the duration of Seduction
...
/*void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)*/ //old one
{
    if (m_CastItem)
...
void Spell::_handle_immediate_phase()
{
	...
	// Fixed Glyph of Totem of Wrath and Entrapment procs:
	// https://github.com/TrinityCore/TrinityCore/commit/1a06e197fd7d908ff62b801176cc3797306b07cd 
	if (!m_originalCaster)
        return;
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {	
			float ox = target->GetPositionX();
		    float oy = target->GetPositionY();
		    float oz = target->GetPositionZ();
			...
			if (m_caster->GetEntry() != WORLD_TRIGGER) // Ignore LOS for gameobjects casts (wrongly casted by a trigger)
		        if (!(m_spellInfo->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) && VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) && !m_caster->IsWithinLOS(ox, oy, oz))
	...
	if (m_targets.HasDst())
    {
		float ox = m_targets.GetDstPos()->GetPositionX();
		float oy = m_targets.GetDstPos()->GetPositionY();
		float oz = m_targets.GetDstPos()->GetPositionZ();
	
        if (!(m_spellInfo->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) && VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) && !m_caster->IsWithinLOS(ox, oy, oz))
            return SPELL_FAILED_LINE_OF_SIGHT;

		// Mass Dispel and Shadowfury should return SPELL_FAILED_LINE_OF_SIGHT 
		// when caster is not within los by Natureknight
		switch (m_spellInfo->Id)
		{
	...
	// TEMP SOLUTION FOR FEAR VS RED JUDGEMENT...
	if (m_caster->HasUnitState(UNIT_STATE_FLEEING | UNIT_STATE_FLEEING_MOVE))
		if (m_spellInfo->Id == 42292 || m_spellInfo->Id == 59752) // PvP Trinket spells
			if (m_caster->HasAura(20184)) // Red Judgement debuff
	            return SPELL_CAST_OK;

	if (!(m_spellInfo->AttributesEx5 & SPELL_ATTR5_USABLE_WHILE_FEARED))
		if (m_caster->HasAura(20184) && m_caster->HasUnitState(UNIT_STATE_FLEEING | UNIT_STATE_FLEEING_MOVE)) // Red Judgement debuff
			return SPELL_FAILED_FLEEING;

	// ...AND MIND CONTROL WHEN SHADOWFIEND PRESENT
	if (m_spellInfo->Id == 605) // Mind Control
	{
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                m_preGeneratedPath.calculate(pos.m_positionX, pos.m_positionY, pos.m_positionZ + target->GetObjectSize());

                if (m_preGeneratedPath.getPathType() & PATHFIND_SHORT || m_preGeneratedPath.getPathType() & PATHFIND_NOPATH)
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            case SPELL_AURA_DUMMY:
            {
                ...
                switch (m_spellInfo->Id)
                {
					// Wild Growth should not be castable while target is not within LoS
					// but the aura should not respect los
					...
					{
						Unit* target = m_targets.GetUnitTarget();
						if (m_caster != target && !m_caster->IsWithinLOSInMap(target))
							return SPELL_FAILED_LINE_OF_SIGHT;
					}
					break;
					// Savage Roar should be able to use only in cat form
...
uint32 Spell::GetCCDelay(SpellInfo const* _spell)
{
	...
	switch(_spell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            if (_spell->Id == 49576) // Death Grip
                return delayForInstantSpells;
            break;
		// Fix pet HP after summoning the same pet (need to apply also sql-s):
		case SPELLFAMILY_WARLOCK:
			if (_spell->Id == 712 || // Summon succubus
				_spell->Id == 691 || // Summon Felhunter
				_spell->Id == 697 || // Summon Voidwalker
				_spell->Id == 688)   // Summon Imp
				return 30;
			break;
		case SPELLFAMILY_PALADIN:
			if (_spell->Id == 67485) // HoR Trigger damage
				return 200;
	}
...
SpellCastResult Spell::CheckCasterAuras() const
{
    ...
    // Glyph of Pain Suppression by Natureknight
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_PRIEST && m_spellInfo->SpellIconID == 2178) // Pain Suppression
	{
		if (m_caster->HasAura(63248)) // "immune" to stun horror and freeze only for this cast
            mechanic_immune = (1 << MECHANIC_STUN) | (1 << MECHANIC_FREEZE) | (1 << MECHANIC_HORROR);
		else usableInStun = false;
	}
	
	if (m_spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN && m_spellInfo->Id == 1044) // Hand of freedom
		mechanic_immune = (1 << MECHANIC_SAPPED) | (1 << MECHANIC_HORROR);
	
	if (m_spellInfo->Id == 47585) // Dispersion (Priest)
		mechanic_immune = (1 << MECHANIC_SILENCE) | (1 << MECHANIC_FEAR) | (1 << MECHANIC_STUN) | (1 << MECHANIC_HORROR);
	
	if (m_spellInfo->Id == 22812) // Barkskin (Druid)
		mechanic_immune = 1 << MECHANIC_HORROR;

	if (m_spellInfo->Id == 51490) // Thunderstorm (Shaman)
		mechanic_immune = (1 << MECHANIC_SAPPED) | (1 << MECHANIC_FREEZE) | (1 << MECHANIC_STUN);
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff) const
{
	...
	// Added LoS check between targets and the destination center point of AoE spells
    // targets from AoE spells should be LOS checked from the destination center point
    if (target != m_caster && m_targets.HasDst())
    {
        float x, y, z;
    ...
    switch (m_spellInfo->Effects[eff].Effect)
    {
        ...
        default:                                            // normal case
			...
			{
				// Those spells should not deal damage trought LoS
				switch (m_spellInfo->Id)
				{
			...
			// FIX FOR LOS EATING SPELLS LIKE A DAWG
			// PROBABLY THE PROPER WAY BY NATUREKNIGHT
			// SHOULD RETURN TRUE INSTEAD OF FALSE OR
			// THIS LAST ROW IS NOT NECESSARY AT ALL
			return true;
			}
            break;
    }
